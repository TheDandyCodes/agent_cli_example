...
def process_data(
    ...
    def fetch_sample_data(start_date: str, end_date: str) -> bpd.DataFrame:
        """Fetch documentation from local markdown files."""
        import os
        import pandas as pd
                ...
                if filename.endswith(".md"):
                    with open(os.path.join(docs_dir, filename), "r") as f:
                        ...
                        data.append({
                            ...
                            "answers": [] # No separate answers for raw docs
                        })
        
        if not data:
            logging.warning(f"No markdown files found in {docs_dir}")
            # Ensure dtypes match expected schema even if empty
            df = pd.DataFrame(columns=["creation_date", "last_edit_date", "question_id", "question_title", "question_text", "answers"])
            df = df.astype({
                "question_id": "string",
